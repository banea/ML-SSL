...
public class MonolingualCoTrainingML {
  ...
  private static final double CONFIDENCE = 0.53; // confidence has to be in the
                                                 // 0-1.0 range
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int fold = 0; fold < MAXFOLDS; fold++) {
      for (int iteration = 0; iteration <= MAXITERATIONS; iteration++) {
        ...
        // English
        BufferedReader readerEn = new BufferedReader(new FileReader(datapath
        ...
        // Romanian
        BufferedReader readerRo = new BufferedReader(new FileReader(datapath
            + FS + TYPE + "-test-" + LANGUAGES[1] + "-" + fold + "-"
            + iteration + ".arff"));
        ...
        saverEnTest.setFile(new File(datapath + FS + TYPE + "-test-"
        ...
        saverRoTest.setFile(new File(datapath + FS + TYPE + "-test-"
            + LANGUAGES[1] + "-" + fold + "-" + nextiteration + ".arff"));
        ...
        while ((instEn = arffEn.readInstance(dataEn)) != null
            && (instRo = arffRo.readInstance(dataRo)) != null) {
          instance_counter++;
          ...
          InstanceValuePair to_be_considered_instance_value_pair = new InstanceValuePair(
              instEn, instRo, average_score, label);
          ...
          if (to_be_added_to_test_set_instance_value_pair != null) {
            if (label == 0) {
              System.out.println("Instance " + instance_counter
                  + " enClsLabel[0]=" + enClsLabel[0] + " enClsLabel[1]="
                  + enClsLabel[1] + " roClsLabel[0]=" + roClsLabel[0]
                  + " roClsLabel[1]=" + roClsLabel[1]);
              System.out.println(to_be_considered_instance_value_pair
                  .getValue());
              System.out.println(to_be_added_to_test_set_instance_value_pair
                  .getValue());
              System.out.println("Best subj priority queue: "
                  + toStringPriorityQueue(bestSubj));
              System.out.println("Best obj priority queue: "
                  + toStringPriorityQueue(bestObj));
            }
            testcount++;
            // sets the dataset for the instances to be written and writes them
            // to the test files
            Instance testEnInstance = to_be_added_to_test_set_instance_value_pair
                .getLang1Instance();
            testEnInstance.setDataset(dataEn);
            saverEnTest.writeIncremental(testEnInstance);

            Instance testRoInstance = to_be_added_to_test_set_instance_value_pair
                .getLang2Instance();
        ...
        System.out.println("Number of test instances for iteration "
            + nextiteration + " :" + testcount);
        ...
        System.out.println("Found objective examples: " + bestObj.size()
  ...
  private static void updateTrainSet(PriorityQueue<InstanceValuePair> bestObj,
    ...
    // English
    BufferedReader readerEn = new BufferedReader(new FileReader(datapath + FS
    ...
    Instance instEn;
    // Romanian
    BufferedReader readerRo = new BufferedReader(new FileReader(datapath + FS
        + TYPE + "-train-" + LANGUAGES[1] + "-" + fold + "-" + iteration
        + ".arff"));
    ArffReader arffRo = new ArffReader(readerRo, 1000);
    Instances dataRo = arffRo.getStructure();
    dataRo.setClassIndex(dataRo.numAttributes() - 1);
  ...
  private static Classifier trainClassifier(String language, int fold,
    ...
    Instances curTrain = loadDataset(datapath + FS + TYPE + "-train-"
        + language + "-" + fold + "-" + iteration + ".arff");
    // set class attribute
    curTrain.setClassIndex(curTrain.numAttributes() - 1);

    // initialize classifier
    // weka.classifiers.bayes.NaiveBayes classifier = new
    ...
    try {
      classifier
          .setOptions(weka.core.Utils
              .splitOptions("weka.classifiers.functions.LibSVM -S 0 -K 2 -D 3 -G 0.0 -R 0.0 -N 0.5 -M 40.0 -C 1.0 -E 0.0010 -P 0.1"));
    } catch (Exception ex) {
      Logger.getLogger(MonolingualCoTrainingML.class.getName()).log(
  ...
  private static void evaluateClassifier(Classifier classifier,
    ...
    try {
      ...
      System.out.println(eval.toClassDetailsString("@ " + language + " "
          + iteration + "-" + fold + "-class bootstrapping"));
      System.out.println(eval.toSummaryString("@ " + language + " " + iteration
          + "-" + fold + "-overall bootstrapping", false));
      output.write(eval.toClassDetailsString("@ " + language + " " + iteration
          + "-" + fold + "-class bootstrapping")
          + "\n");
      output.write(eval.toSummaryString("@ " + language + " " + iteration + "-"
          + fold + "-overall bootstrapping", false)
          + "\n");
    } catch (Exception ex) {
      Logger.getLogger(MonolingualCoTrainingML.class.getName()).log(
