...
public class RegressionTesting {
  ...
  public static void main(String[] args) throws Exception {
    ...
    output = new BufferedWriter(new FileWriter (testdatapath+FS+"results-"+DEVIATION+"-"+LASTITERATION+".txt"));
    for (int fold = 0; fold < MAXFOLDS; fold++) {
      ...
      Classifier enClassifier = trainClassifier(LANGUAGES[0], fold, LASTITERATION);
      toLog("entering Romanian trainClassifier function");
      Classifier roClassifier = trainClassifier(LANGUAGES[1], fold, LASTITERATION);
    }
    output.close();
  }
  ...
  private static Classifier trainClassifier(String language, int fold, int iteration) {
    ...
    Instances curTrain = loadDataset(traindatapath +FS+TYPE+"-train-" + language + "-" + fold + "-" + iteration + ".arff", "train");
    //set class attribute
  ...
  private static void evaluateClassifier(Classifier classifier, String language, int fold, int iteration, Instances train) {
    Instances testset = loadDataset(testdatapath +"label-"+FS+TYPE+"-test-" + language + ".arff","labeled-test");
    ...
    for (int i=0; i<CLASSES.length; i++){
      ...
      System.out.println(results);
        ...
        //Logger.getLogger(MonolingualRegressionTesting.class.getName()).log(Level.SEVERE, null, ex);
      }
