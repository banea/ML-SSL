...
public class MonolingualCoTrainingRegression {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int fold = 0; fold < MAXFOLDS; fold++) {
      ...
      for (int iteration = 11; iteration <= MAXITERATIONS; iteration++) {
        ...
        // English
        toLog("opens English Arff Savers for next iteration (i.e. #"
        ...
        // Romanian
        toLog("opens Romanian Arff Savers for next iteration (i.e. #"
            + nextiteration + ") test data");
        ...
        while ((instEn = arffEn.readInstance(dataEn)) != null
          ...
          if (enClsLabel < objRegThreshold && roClsLabel < objRegThreshold) { // sample
                                                                              // belongs
                                                                              // to
                                                                              // the
                                                                              // objective
          ...
          } else if (enClsLabel > subjRegThreshold
              && roClsLabel > subjRegThreshold) { // sample belongs to the
                                                  // subjective class
            average_score = (enClsLabel + roClsLabel) / 2;
            if (sampleLabel.equals("mapped")) {
              label = 1.0;
  ...
  private static void evaluateClassifier(Classifier classifier,
    ...
    Instances testset = loadDataset(datapath + "label-" + FS + TYPE + "-test-"
