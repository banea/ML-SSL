...
public class MultilingualCoTrainingRegression {
    ...
    //!!!!modified the MAXITERATIONS number from 10 to 20 to continue from where it left off
    ...
    public static void main(String[] args) throws Exception {
        ...
        //!!!!modified the fold number from 0 to 1 to continue from where it left off
        for (int fold = 2; fold < MAXFOLDS; fold++) {
        	
        	//!!!!modified the iteration number from 0 to 11 to continue from where it left off
        	int iteration = 11;
        	if (fold == 2) {
        		iteration = 18;
        	}
        	...
        	for (; iteration <= MAXITERATIONS; iteration++){
                ...
                BufferedReader readerMult = new BufferedReader(new FileReader(datapath +FS+TYPE+"-test-" + LANGUAGES[0] + "-" + fold + "-" + iteration + ".arff"));
                ...
                toLog("opens multilingual Arff Savers for next iteration (i.e. #" + nextiteration + ") test data");
                ...
                saverMultTest.setFile(new File(datapath + FS + TYPE +"-test-" + LANGUAGES[0] + "-" + fold + "-" + nextiteration + ".arff"));
 
                toLog("entering trainClassifier function");
                //returns trained classifiers and evaluates classifier on labeled test data
                Classifier multClassifier = trainClassifier(LANGUAGES[0], fold, iteration);
                ...
                while ((instMult = arffMult.readInstance(dataMult)) != null) {
                	...
                	double multClsScore = multClassifier.classifyInstance(instMult);
                	 ...
                	 if (multClsScore < objRegThreshold) {	//sample belongs to the objective class
                     	if ( sampleLabel.equals("mapped")){
                     		label = 0.0; //objective score
                     	} else {
                     		label=multClsScore;
                     	}
                     } else if (multClsScore > subjRegThreshold){ //sample belongs to the subjective class
                     	if ( sampleLabel.equals("mapped")){
                     		label = 1.0;
                     	} else {
                    ...
                    InstanceValuePair to_be_considered_instance_value_pair = new InstanceValuePair(instMult, multClsScore, label);
                    ...
                    if (to_be_added_to_test_set_instance_value_pair != null){                     
                    	...
                    	Instance testInstance = to_be_added_to_test_set_instance_value_pair.getInstance();
                ...
                toLog("Number of test instances for iteration "+nextiteration+" :"+testcount);
                output.write("Number of test instances for iteration "+nextiteration+" :"+testcount+"\n");
                saverMultTest.writeIncremental(null);
                toLog("Found objective examples: "+bestObj.size()+" Found subjective examples: "+bestSubj.size()+"\n");
    ...
    private static void updateTrainSet(PriorityQueue<InstanceValuePair> bestObj, PriorityQueue<InstanceValuePair> bestSubj, int fold, int iteration) throws Exception {
        ...
        //English
    	BufferedReader readerMult = new BufferedReader(new FileReader(datapath +FS+ TYPE +"-train-" + LANGUAGES[0] + "-" + fold + "-" + iteration + ".arff"));
        ArffReader arffMult = new ArffReader(readerMult, 1000);
        Instances dataMult = arffMult.getStructure();
        dataMult.setClassIndex(dataMult.numAttributes() - 1);
        Instance instMult;
        ...
        saverMultTrain.setFile(new File(datapath +FS +TYPE+"-train-" + LANGUAGES[0] + "-" + fold + "-" + nextiteration + ".arff"));
    ...
    private static Classifier trainClassifier(String language, int fold, int iteration) {
        ...
        /*weka.classifiers.functions.SMOreg classifier = new weka.classifiers.functions.SMOreg();
        try {
        	classifier.setOptions(weka.core.Utils.splitOptions("-C 1.0 -N 0 -I \"weka.classifiers.functions.supportVector.RegSMOImproved -L 0.0010 -W 1 -P 1.0E-12 -T 0.0010 -V\" -K \"weka.classifiers.functions.supportVector.PolyKernel -C 250007 -E 1.0\""));
        	} catch (Exception ex) {
        		ex.toString();
            Logger.getLogger(multilingual.class.getName()).log(Level.SEVERE, null, ex);
        }*/
        ...
        try {
            ...
            Logger.getLogger(MultilingualCoTrainingRegression.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private static void evaluateClassifier(Classifier classifier, String language, int fold, int iteration, Instances train) {
        Instances testset = loadDataset(datapath +FS+"label-"+TYPE+"-test-" + language + "-"+fold+".arff","labeled-test");
